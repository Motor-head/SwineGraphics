...
rem  --class-path "target\installer\input\libs\*" ^
"%JAVA_HOME%\bin\jdeps" ^
  -q ^
  --multi-release %JAVA_VERSION% ^
  --ignore-missing-deps ^
  --class-path "target\libs\*" ^
  --print-module-deps target\classes\%MODULE_PATH%\App.class > temp.txt
...
call "%JAVA_HOME%\bin\jpackage" ^
  ...
  --main-jar %MAIN_JAR% ^
  --java-options "-Xmx2048m --enable-native-access=javafx.graphics" ^
